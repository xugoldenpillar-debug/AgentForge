...
> 2026-09-07 状态注：下文为 2026-09-06 导出审计快照，关于工作区提交历史、未跟踪锁文件和观察依赖版本的描述不是当前状态。当前 `pnpm-lock.yaml` 已跟踪、Pi core 已钉住；此变化不自动完成导出/许可审计，不修改下面历史观察值。当前安装入口见 [根 README](../../README.md)。

日期：2026-09-06。本文是 T0 的审计证据与待审清单及公开发布清单草案，不是批准的 release manifest、仓库开源声明、许可证授予或法律意见。它只描述当前 AgentForge checkout 中可观察到的来源、贡献、历史和依赖证据；公开仓库、公开产物、拆仓和推送仍需单独授权。
...
- 当前工作区的 community contract 实现尚未进入可审计的提交历史；
...
- 当前没有提交 `pnpm-lock.yaml`，依赖的观察版本不是可复现的发布证据；
...
| --- | --- | --- | --- | --- | --- |
| `src/lib/community/index.ts` | 社区契约入口，重新导出定义解析和公开投影能力 | 第一方工作区文件；当前为本地未提交改动，尚无独立贡献声明或 CLA 证据 | 当前可达 Git 历史中未形成社区组件库提交 | 仓库无根 `LICENSE`；不能推定复用许可 | 计划公开；当前 public dependency audit 已无 blocker，仍需完成授权审查 |
| `src/lib/community/contracts.ts` | v1 `workflow-recipe`／`instruction-skill` envelope、附件和公开投影契约 | 第一方工作区文件；当前为本地未提交改动 | 当前可达 Git 历史中未形成社区组件库提交 | 同上；代码可见不等于获得复用许可 | 计划公开；当前 public dependency audit 已无 blocker，仍需完成发布审查 |
...
| --- | --- | --- | --- | --- |
| `src/lib/community/index.ts` | 公开契约入口 | 是（条件） | 当前工作区新增／未提交；无独立作者权利链记录 | 待补充第一方许可与贡献证据 |
| `src/lib/community/contracts.ts` | 公开契约解析／投影 | 是（条件） | 当前工作区新增／未提交；无独立作者权利链记录 | public dependency audit 已无 blocker；仍待完成独立导出、许可与历史审查 |
...
以下表格来自当前 `package.json` 的直接依赖声明和本地 `node_modules/*/package.json` 的观察值。`requested` 是声明范围，`observed` 是本次工作区已安装版本；仓库没有提交 `pnpm-lock.yaml`，所以 observed 不能作为可复现构建锁定。source 是包元数据中的 repository 字段，license 是包元数据声明；两者均需人工核对许可证全文、NOTICE、传递依赖、版本和导出用途。
...
- no committed lockfile, so caret ranges require reproducibility review and observed versions cannot serve as release evidence;
