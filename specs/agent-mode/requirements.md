...
| AM2 | 复用 Build/不可变 BuildVersion/竞技 Run/Submission；固定不可变 ComponentVersion；保存做 expectedVersionId 并发校验 | 保存冲突、运行中改草稿、组件发布新版本均不得替换执行快照 |
...
| AM6 | 不可信 Python 仅在 Next.js 进程之外的隔离沙箱；无宿主 shell、宿主目录或生产网络/秘密 | 独立 Worker 本身不等于沙箱；拒绝挂载 Docker socket、宿主凭据和任意出站 |
...
| AM12 | 首个 MVP 仅单 Agent Python CSV+JSON+报告；所有开放都需对应阶段证据 | 文档、Pi 单次样本、离线测试不能代替真实沙箱/浏览器/竞技验收 |
