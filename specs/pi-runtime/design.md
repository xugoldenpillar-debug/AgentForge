...
上游固定快照的仓库清单为 `@earendil-works/pi-agent-core` / `@earendil-works/pi-coding-agent`，版本 `0.85.1`，Node `>=22.19.0`；这不是已经核验的 npm 最新发布版本。当前 AgentForge 声明 Node `>=22.16.0`。PI0 必须确认实际可安装产物、锁文件、引擎与传递依赖，不能照抄旧 scope 或静默抬高 Portable 最低版本。

建议 Pi 放可选服务端边界，明确检查运行条件，不满足就拒绝启用 Pi，而不是整个 DAG／Portable 不可运行。是否调整全栈 Node 基线在确切依赖版本确定后单独说明并验证 CI。
...
- 用户可选 Pi 的具体发布时点受待决策 D3 控制；PoC 默认不对公共用户开放。
...
组件自测与现有请求绑定 Run 的差异必须明确。首版采用何种持久化调度由任务 T5 决定，不能拿内存异步函数冒充可恢复 Worker。
...
- 网络断线后状态查询只读，不触发新运行。
- 生产长任务需租约／心跳／过期状态处理；此项未实现前仅允许受限试点，明确中断语义。
...
- `src/server/runtime/pi/`：Pi 适配、模型桥接；可选加载，不能进入 Portable 原生导入树。
