import type { Config, NodeKind, Workflow } from './workflow-types.ts';
import { ensure, ERROR_CODES, withErrorCode } from './error-core.ts';
import { hasSkillId, hasToolId } from './workflow-catalog.ts';
...
function validateWorkflowInternal(value: unknown): Workflow {
  ...
  const nodes = value.nodes.map((raw:unknown) => {
    ...
    if(kind==='skill') ensure(hasSkillId(c.skillId),'Unknown skill.');
    if(kind==='tool') ensure(hasToolId(c.toolId),'Unknown tool.');
