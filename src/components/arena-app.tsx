  ...
  return (
    <div className="hero-lab" aria-label={t('home.workflowIllustration')}>
      <div className="lab-titlebar">
        ...
        <span className="push-right accent">{t('common.simulated').toUpperCase()}</span>
      </div>
      <div className="lab-stage">
        ...
        <div className="mini-node model">
          ...
          <div className="mini-name">demo-forge</div>
      ...
      <div className="home-main">
        <div>
          ...
          <div className="card boss-card">
            ...
            <div className="flex justify-between small mb-2"><span className="muted">{t('common.communitySolveRate')} <span className="dim">/ {t('common.demoLane')}</span></span><span className="mono purple">{formatPercent(boss.stats.solveRate)}</span></div>
...
function LeaderboardSection({ problemId }: { problemId?: string }) {
  ...
  const [tier, setTier] = useState(query.get('tier') || 'demo');
  ...
  return <><div className="leaderboard-controls"><div className="tabs" style={{ margin: 0, padding: 0, border: 0 }}>{sortOptions.map(([value, icon, label]) => <button key={value} className={cn('tab', sort === value && 'active')} onClick={() => setSort(value)}><Icon name={icon} />{label}</button>)}</div><select aria-label={t('leaderboard.filterChallenge')} value={tier} onChange={(event) => setTier(event.target.value)}><option value="demo">{t('leaderboard.demoLane')}</option><option value="byok">{t('leaderboard.byokLane')}</option><option value="verified">{t('leaderboard.verifiedLane')}</option></select></div><LaneNote tier={tier} /><div className="mt-3">{loading ? <Loading /> : error ? <ErrorNotice message={error} retry={reload} /> : <LeaderTable rows={data ?? []} />}</div><p className="small dim mt-2">{t('challenge.scoringNote')}</p></>;
}
...
function FailureLab({ problem, onSubmitted }: { problem: any; onSubmitted: () => void }) {
  ...
  return <div className="detail-grid"><div className="stack">
    <div className="card card-pad"><div className="flex items-center gap-2 mb-2"><Icon name="Crosshair" className="purple" /><h3>{t('failure.title')}</h3></div><p className="muted small mb-3">{t('failure.publicBuildHint')}</p>
      {user ? <form onSubmit={submit}><div className="field"><label className="label" htmlFor="failure-input">{t('failure.input')}</label><textarea id="failure-input" required maxLength={4000} rows={5} value={input} onChange={(event) => setInput(event.target.value)} placeholder={t('failure.input')} /></div><div className="field"><label className="label" htmlFor="failure-reason">{t('failure.reason')}</label><textarea id="failure-reason" required minLength={8} maxLength={1000} rows={3} value={reason} onChange={(event) => setReason(event.target.value)} placeholder={t('failure.reason')} /></div><div className="flex gap-2"><select aria-label={t('failure.providerLabel')} value={provider} onChange={(event) => setProvider(event.target.value)}>{providers.data?.demo && <option value="demo">{t('failure.demoEngine')}</option>}{providers.data?.platform && <option value="platform">{t('failure.platformGateway')}</option>}{providers.data?.credentials?.map((credential: any) => <option key={credential.id} value={credential.id}>{credential.name}</option>)}</select><Button type="submit" variant="default" disabled={busy}>{busy ? <span className="spinner" /> : <Icon name="Crosshair" />}{t('failure.testTopAgent')}</Button></div></form> : <Link href={`/login?next=/challenges/${problem.slug}`} className="button primary">{t('failure.signIn')} <Icon name="ArrowRight" /></Link>}
...
function Providers() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      await post('providers', { name: values.get('name'), baseUrl: values.get('baseUrl'), apiKey: values.get('apiKey'), modelId: values.get('modelId'), inputPrice: values.get('inputPrice') ? Number(values.get('inputPrice')) : null, outputPrice: values.get('outputPrice') ? Number(values.get('outputPrice')) : null });
      form.reset();
...
function BuildDetail({ id }: { id: string }) {
  ...
  const laneLabel = best?.tier === 'byok' ? t('leaderboard.byokLane') : best?.tier === 'verified' ? t('leaderboard.verifiedLane') : t('leaderboard.demoLane');
  const fork = async () => {
    if (!user) {
