...
export function serializeRun(run:Run,cases:RunCase[]=[]){
  const base={id:run.id,buildId:run.buildId,versionId:run.versionId,kind:run.kind,tier:run.tier,status:run.status,createdAt:run.createdAt,summary:run.summary};
