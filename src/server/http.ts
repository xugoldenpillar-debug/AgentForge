import { AppError, ensure, safeError } from '../shared/errors.ts';
import type { ArenaService } from './service.ts';
export async function readJson(request:Request):Promise<Record<string,unknown>>{
  ensure(request.headers.get('content-type')?.includes('application/json'),'Use application/json.');
  const reader=request.body?.getReader();ensure(reader,'Request body is required.');let size=0;const chunks:Uint8Array[]=[];
  for(;;){const {done,value}=await reader.read();if(done)break;size+=value.byteLength;if(size>128*1024){await reader.cancel();throw new AppError('Request body exceeds 128 KB.',413);}chunks.push(value);}
  let parsed:unknown;try{parsed=JSON.parse(Buffer.concat(chunks).toString('utf8'));}catch{throw new AppError('Invalid JSON body.');}
  ensure(parsed&&typeof parsed==='object'&&!Array.isArray(parsed),'Request body must be an object.');return parsed as Record<string,unknown>;
}
...
export async function handleArena(request:Request,options:{service:ArenaService;userId?:string;origin:string;validateBody?:(path:string,body:Record<string,unknown>)=>void}):Promise<Response>{
  ...
  try{
    ...
    const mutating=method!=='GET';if(mutating){
      const origin=request.headers.get('origin');ensure(!origin||origin===options.origin,'Cross-origin mutations are not allowed.',403);
      ensure(request.headers.get('sec-fetch-site')!=='cross-site','Cross-site mutations are not allowed.',403);
      ensure(userId,'Sign in to continue.',401);await service.limit(userId,'api',120);
    }
    const auth=()=>{ensure(userId,'Sign in to continue.',401);return userId;};
    if(method==='GET'){
      ...
      if(path[0]==='skills')return json(await service.skills());if(path[0]==='tools')return json(await service.tools());
    ...
    if(method==='POST'){
      const body=await readJson(request);options.validateBody?.(path.join('/'),body);
      ...
      if(path[0]==='runs'){
        const uid=auth(),encoder=new TextEncoder(),abort=new AbortController();let closed=false;
        const stream=new ReadableStream<Uint8Array>({
          async start(controller){
            const push=(event:unknown)=>{if(!closed)try{controller.enqueue(encoder.encode(JSON.stringify(event)+'\n'));}catch{closed=true;abort.abort();}};
            try{await service.run(uid,body,push,AbortSignal.any([abort.signal,request.signal,AbortSignal.timeout(180000)]));}
            catch(e){push({type:'error',message:safeError(e).message});}
  ...
  }catch(error){const safe=safeError(error);return json({error:safe.message},safe.status);}
}
