...
import type { AIProvider, AIRequest, AIResult, ProviderResolver } from '../../lib/ai/types.ts';
...
class InvocationRecordingProvider implements AIProvider {
  readonly id: string;
  ...
  async execute(request: AIRequest): Promise<AIResult> {
    ...
    try {
      ...
      try {
        ...
        // The original provider call is already outside our process boundary.
        // Preserve the conservative unknown outcome instead of retrying it.
      }
...
class UnknownProviderResultError extends Error {
