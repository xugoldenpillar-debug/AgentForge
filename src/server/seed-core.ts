import type { Repository, User, Workflow, Submission, CaseResult } from '../shared/types.ts';
import { SKILLS,TOOLS,BADGES,starterWorkflow,JSON_SCHEMA } from '../shared/catalog.ts';
import { PROBLEMS,TEST_CASES } from './fixtures.ts';
import { summarize } from '../lib/scoring/index.ts';
const now='2026-09-01T08:00:00.000Z';
export async function seedCore(repo:Repository,demoUser?:User):Promise<void>{
  ...
  await repo.transaction(async tx=>{
    ...
    for(let i=0;i<20;i++){
      ...
      let workflow:Workflow=starterWorkflow(problem.judge);
