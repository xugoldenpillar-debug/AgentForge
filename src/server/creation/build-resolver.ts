...
/**
 * Authoritative resolver for the deliberately small public creation challenge lane.
 * Provider credentials remain a separate run-time authorization and are never
 * persisted in the immutable Build definition.
 */
export function createCreationAgentBuildResolver(repository?: Repository): AgentBuildResolver {
  return Object.freeze({
    async resolve(request: AgentBuildResolutionRequest) {
      const definition = parseAgentBuildDefinition(request.definition);
        ...
        'This launch uses fixed sandbox capabilities; Skill uploads do not grant execution rights.',
        409, ERROR_CODES.RUNTIME_POLICY_DENIED);
      if (definition.skillRefs.length) {
        ensure(repository && request.actorId === request.ownerId,
          'Private Skills are not available to this actor.', 403, ERROR_CODES.OWNERSHIP_FORBIDDEN);
        await resolveCreationSkills(repository, request.ownerId, definition.skillRefs);
      }
...
export const creationAgentBuildResolver = createCreationAgentBuildResolver();
