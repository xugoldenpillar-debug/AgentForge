...
export class ChallengeApplicationService {
  ...
  async review(reviewerId: string, applicationId: string, input: { decision: unknown; reason: unknown }): Promise<ExtensionApplication> {
    ...
    ensure(input.decision === 'approved' || input.decision === 'rejected', 'A valid review decision is required.', 400, ERROR_CODES.REQUEST_VALIDATION_FAILED);
    ...
    await this.#repository.transaction(async (tx) => {
      ...
      }, { status: input.decision, updatedAt: now, decidedAt: now });
