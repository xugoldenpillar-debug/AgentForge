...
export function validateBody(path:string,body:Record<string,unknown>){const schema=schemas[path];if(schema&&!schema.safeParse(body).success)throw new AppError('Invalid request fields. Check the form values and length limits.');}
