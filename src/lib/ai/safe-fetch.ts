...
export function safeProviderFetch(baseUrl:string):typeof fetch {
  ...
  if(!agent){
    agent=new Agent({connections:8,connect:{lookup:(hostname:string,options:any,callback:any)=>{
      ...
      }).catch(()=>callback(new Error('Provider network policy rejected the connection.')));
    }}});agents.set(base.hostname,agent);
  }
  return (async(input:RequestInfo|URL,init?:RequestInit)=>{
    const url=new URL(typeof input==='string'?input:input instanceof URL?input.href:input.url);
    if(url.origin!==base.origin||!url.pathname.startsWith(base.pathname.replace(/\/$/,'')+'/'))throw new AppError('Provider network policy rejected the request.',400);
    ...
    const body=response.body.pipeThrough(new TransformStream<Uint8Array,Uint8Array>({transform(chunk,controller){size+=chunk.byteLength;if(size>2*1024*1024)throw new Error('Provider response exceeds the size limit.');controller.enqueue(chunk);}}));
