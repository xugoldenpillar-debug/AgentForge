...
import { AppError,ensure,safeError } from '../src/shared/errors.ts';
...
async function handleAuth(request:Request,origin:string,clientIp:string){
 ...
 ensure(request.method==='POST','Method not allowed.',405);
 ensure(!request.headers.get('origin')||request.headers.get('origin')===origin,'Cross-origin authentication is not allowed.',403);
 ensure(request.headers.get('sec-fetch-site')!=='cross-site','Cross-site authentication is not allowed.',403);
 ensure(await repo.rateLimit(`auth:${clientIp}`,30,60000),'Too many authentication attempts. Try again later.',429);
 ...
 ensure(path==='/api/auth/sign-up/email'||path==='/api/auth/sign-in/email','Authentication endpoint not available in the offline runtime.',404);
 ensure(typeof body.email==='string'&&body.email.length<=254&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email),'Enter a valid email address.');
 ensure(typeof body.password==='string'&&body.password.length>=10&&body.password.length<=128,'Password must contain 10 to 128 characters.');
 const email=body.email.trim().toLowerCase(),password=body.password;
 if(path==='/api/auth/sign-up/email'){
   ensure(typeof body.name==='string'&&body.name.trim().length>=2&&body.name.length<=60,'Name must contain 2 to 60 characters.');
   ...
   await repo.transaction(async tx=>{ensure(!(await tx.read('users',{email})).length,'An account with this email already exists.',409);await tx.insert('users',[user]);});
 ...
 ensure(timingSafeEqual(candidate,stored)&&account,'Invalid email or password.',401);
 const user=(await repo.read('users',{id:account.userId}))[0];ensure(user,'Invalid email or password.',401);return newSession(user);
}
...
const assets:Record<string,{file:string;type:string}>={'/arena.css':{file:'arena.css',type:'text/css; charset=utf-8'},'/portable-app.js':{file:'portable-app.js',type:'text/javascript; charset=utf-8'},'/favicon.svg':{file:'favicon.svg',type:'image/svg+xml'}};
const html='<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#0b0f13"><title>AgentForge | Build AI. Beat Problems.</title><meta name="description" content="Build agents. Challenge real problems. An offline-playable AI arena."><link rel="icon" href="/favicon.svg"><link rel="stylesheet" href="/arena.css"></head><body><div id="app"><main class="container page"><div class="eyebrow">INITIALIZING THE FORGE</div><h1>AgentForge</h1></main></div><div class="toast-container" id="toasts" aria-live="polite"></div><script src="/portable-app.js" defer></script></body></html>';
...
const server=createServer(async(req,res)=>{
 ...
 try{
   const host=req.headers.host||'';ensure(host===`localhost:${port}`||host===`127.0.0.1:${port}`,'Invalid Host. This demo accepts localhost only.',403);
   const origin=`http://${host}`,url=new URL(req.url||'/',origin);let body:Buffer|undefined;
   if(!['GET','HEAD'].includes(req.method||'GET')){let size=0;const chunks:Buffer[]=[];for await(const chunk of req){size+=chunk.length;ensure(size<=128*1024,'Request body exceeds 128 KB.',413);chunks.push(chunk);}body=Buffer.concat(chunks);}
 ...
 }catch(e){const safe=safeError(e);if(!res.headersSent)res.writeHead(safe.status,{...securityHeaders,'Content-Type':'application/json','Cache-Control':'no-store'});if(!res.writableEnded)res.end(JSON.stringify({error:safe.message}));}
});
