...
async function fixture(env: Record<string, string | undefined> = {}) {
  ...
  const service = new ArenaService(repo, {
    ...
    env: { APP_ENV: 'test', DEMO_MODE: 'true', PI_RUNTIME_ENABLED: 'true', ...env }
  });
  return { repo, service };
}
...
test('real Pi self-test fails closed before provider access until EF accounting exists', async () => {
  ...
  try {
    ...
    assert.equal(events.at(-1).code, ERROR_CODES.RUNTIME_UNAVAILABLE);
...
test('Pi status and execution fail closed with EF or outside explicit standalone test mode', async () => {
  for (const env of [
    {EVALUATION_SCHEDULER_MODE: 'outbox'},
    {APP_ENV: 'production'},
    {DEMO_MODE: 'false'},
  ]) {
    const {service, repo} = await fixture({...env, PI_RUNTIME_INVITED_EMAILS: user.email});
    const status = await service.piSelfTestStatus(user.id);
    assert.equal(status.canRun, false);
    assert.equal(status.reason, 'accounting_unavailable');
    assert.deepEqual(await service.applyPiSelfTest(user.id), status);
    let leases = 0;
    repo.lease = async () => { leases++; throw new Error('No legacy admission allowed'); };
    for (const credentialId of ['demo', 'real-credential']) {
      await assert.rejects(service.runPiSelfTest(user.id, {prompt: '2+3', credentialId}, () => {
        assert.fail('No execution events allowed');
...
test('configured EF scheduler alone disables Pi before a legacy lease or model call', async () => {
  const {repo} = await fixture();
  let calls = 0;
  const service = new ArenaService(repo, {
    demoMode: true, encryptionKey: randomBytes(32).toString('base64'), allowedHosts: [],
    env: {APP_ENV: 'test', DEMO_MODE: 'true', PI_RUNTIME_ENABLED: 'true', PI_RUNTIME_INVITED_EMAILS: user.email},
    competitiveRunScheduler: createDurableOutboxCompetitiveRunScheduler(repo),
    createRealProvider: () => { calls++; throw new Error('No provider access'); },
  });
  repo.lease = async () => { calls++; throw new Error('No legacy lease'); };
  assert.equal((await service.piSelfTestStatus(user.id)).canRun, false);
  await assert.rejects(service.runPiSelfTest(user.id, {prompt: '2+3'}, () => {
    assert.fail('No stream execution');
  }), {code: ERROR_CODES.RUNTIME_UNAVAILABLE});
