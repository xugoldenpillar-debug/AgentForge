...
test('saved Agent Build reads preserve the exact animation challenge version for session recovery', async () => {
  const { repo, service } = await fixture();
  const challenge = ANIMATION_CHALLENGES[0];
  const challengeVersion = ANIMATION_CHALLENGE_VERSIONS.find((version) => version.challengeId === challenge.id)!;
  await repo.insert('animationChallenges', [{ ...challenge }]);
  await repo.insert('animationChallengeVersions', [{ ...challengeVersion }]);
  const saved = await service.saveBuild('owner', {
    mode: 'agent',
    animationChallengeVersionId: challengeVersion.id,
    title: 'Recoverable Build',
    visibility: 'private',
    agentDefinition: {
      mode: 'agent',
      definitionSchemaVersion: 1,
      instructions: 'Preserve this draft after reload.',
  ...
  const restored = await service.build(saved.id, 'owner', saved.version.id);
  assert.equal(restored.version.animationChallengeVersionId, challengeVersion.id);
  const updated = await service.saveBuild('owner', {
    mode: 'agent',
    buildId: restored.id,
    ...
    visibility: 'private',
    agentDefinition: { ...restored.version.agentDefinition, instructions: 'Recovered draft update.' },
  });
