...
- 新增功能和新增界面必须同时提供简体中文与 English；中英文文案、状态、错误、空态、帮助/提示、可访问性名称与相关测试必须同步更新，禁止只完成单语言。提交前检查 `src/shared/i18n` 的 i18n key 是否齐全、插值参数是否一致，以及缺失 key 的安全 fallback 是否仍然有效。
...
当用户明确提出“快速上线”“功能优先”或同等含义的要求时，可以启用本模式；没有这类明确要求时，继续执行本文件的完整验证模式。

- **阻塞门槛**：开发和上线候选阶段只将 P0/P1 作为阻塞项。P2/P3、文档截图、非关键体验打磨可以延后，但必须在交付说明中逐项记录为“未验证”或“延后处理”，不得把它们描述为已完成。
- **开发中验证**：只运行受影响的定向测试和必要的静态检查，不默认运行全量测试、全栈 smoke 或完整浏览器矩阵。定向测试必须覆盖本次修改的正常路径、错误路径和权限/安全边界（若受影响）。
- **提交前门槛**：提交前至少执行一次 `pnpm typecheck`，并检查 `git diff --check`。若是部署候选，`pnpm build` 只运行一次；不要为了重复确认而反复构建同一候选版本。
- **按变更跳过重型验证**：没有 schema/迁移变化时不跑数据库迁移验证；没有 UI/React 变化时不跑对应浏览器/UI 重型验证；没有 sandbox/runsc 变化时不跑沙箱重型验证；没有依赖、锁文件或 CI 变化时不跑依赖安装一致性或完整 CI 级验证。仍需记录跳过项及原因。
- **安全底线不降级**：本模式不得放宽 API Key 校验和加密/隔离要求、SSRF/Provider 主机与重定向限制、资源所有权与跨用户授权检查、付费或上游未知结果不得重试、HTML/SVG 预览隔离、沙箱网络/资源限制及清理验证。任何触及这些底线的问题仍按 P0/P1 阻塞。
- **正式 release**：正式发布、扩大公开范围或用户明确要求完整验收时，恢复完整验证矩阵，不能用本模式下的定向测试、单次 build、历史截图或 CI 通过替代真实验收。
