...
当需求方明确说“快速上线”“功能优先”或同等含义时，允许按本节缩短验证路径；否则按本指南的完整验证矩阵执行。

- **只阻塞 P0/P1**：快速上线阶段只阻塞 P0/P1 缺陷。P2/P3、文档截图和非关键体验优化可以延期，但 PR/提交说明必须列出具体未验证项、风险和后续处理人或时机；不得将延期项写成已验收。
- **定向验证优先**：开发过程中只运行本次改动直接影响的定向测试；按需补充对应的错误路径、权限边界和安全回归。除非变更范围要求，不默认跑全量测试、全栈 smoke 或完整浏览器验收。
- **提交前固定门槛**：提交前运行 `pnpm typecheck` 和 `git diff --check`。部署候选只运行一次 `pnpm build`，以该次构建产物作为候选依据；构建失败仍然是阻塞问题。
- **无关重型验证可跳过**：无 schema/迁移变化，不跑迁移和数据库升级矩阵；无 UI 变化，不跑对应桌面/窄屏浏览器矩阵；无 sandbox/runsc 变化，不跑沙箱重型验证；无依赖、锁文件或 CI 变化，不跑冻结安装、一致性或完整 CI 级验证。跳过时必须在交付说明中注明“未运行”及理由。
- **不可缩减安全底线**：不得为了快速上线放宽 API Key 格式/权限校验、SSRF 防护、资源所有权和跨用户授权、付费请求未知结果不得重试、HTML/SVG 隔离预览、沙箱网络/资源限制或沙箱清理验证。触及这些底线的缺陷仍然必须阻塞上线，不得降级为 P2/P3。
- **正式 release 恢复完整模式**：正式 release、扩大公开范围、生产安全验收或用户明确要求完整验收时，必须恢复完整测试矩阵和实际运行验收；快速模式的定向测试、单次 build、历史截图或 CI 通过不能替代完整验证。
