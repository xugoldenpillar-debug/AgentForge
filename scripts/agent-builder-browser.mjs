...
try {
  ...
  const label = (key, lang = en) => page.locator('label')
    .filter({ has: page.getByText(lang[key], { exact: true }) })
  ...
  const modelList = page.locator('label').filter({ hasText: en['builderPlus.modelList'] }).locator('select');
  await modelList.waitFor();
  await label('builderPlus.searchModels').fill('quality');
  await modelList.selectOption('builder-quality');
  ...
  await eventually(async () => {
    ...
    assert.equal(saved.buildId, build.id);
    assert.equal(saved.buildVersionId, build.version.id);
  }, 'Saved Build identity was not persisted.');
  ...
  const restoredBuildResponse = page.waitForResponse((response) =>
    response.url().includes(`/api/arena/builds/${build.id}?`) && response.request().method() === 'GET');
  await page.reload({ waitUntil: 'networkidle' });
  const restoredResponse = await restoredBuildResponse;
  assert.equal(restoredResponse.status(), 200, `Saved Build recovery failed: ${restoredResponse.status()}`);
  const restoredBuild = await restoredResponse.json();
  assert.equal(restoredBuild.id, build.id);
  assert.equal(restoredBuild.version.id, build.version.id);
  await eventually(async () => assert.equal(await label('artifactArena.instructions', zhCN).inputValue(), draft), 'Draft did not recover.');
  await eventually(async () => {
    assert.equal(await page.getByText(`Build ${build.id}`, { exact: true }).isVisible(), true);
    assert.equal(await button('artifactArena.forkBuild', zhCN).isVisible(), true);
  }, 'Saved Build state did not recover before editing resumed.');
  assert.equal(await checkbox.isChecked(), true);
  assert.equal(await button('artifactArena.startRun', zhCN).isDisabled(), true);
  ...
  assert.equal(updatedResponse.status(), 201, `Build update failed: ${updatedText}`);
  ...
  assert.equal(await page.locator('section[aria-labelledby="skill-picker-title"] input[type="checkbox"]').count(), 0);
